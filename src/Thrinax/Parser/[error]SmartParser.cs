...
namespace Thrinax.Parser
{
    ...
    public class SmartParser : IParser
    {
        ...
        {
            //输入检查
            if (Response == null || crawlRecode == null)
                return null;
            if (string.IsNullOrEmpty(Response.Content))
                return new ListResponse(Response, Enums.CrawlResult.Empty);
            ...
            HtmlNode htmlNode = HtmlUtility.getSafeHtmlRootNode(Response.Content, true, true);
            ListPagePattern ListPagePattern = new ListPagePattern();
            ListPagePattern.Path = JsonConvert.DeserializeObject<ListPagePattern.Paths>(crawlRecode.ListPattern);

            PageElement[] pageElementList = ExtractContent(Response.Url, htmlNode, ListPagePattern.Path, crawlRecode.LastItemPubDate ?? DateTime.MinValue);
            foreach (PageElement Item in pageElementList)
            {

                if (!string.IsNullOrWhiteSpace(crawlRecode.LastItemID))
                {
                    if (crawlRecode.LastItemID == MD5Helper.getMd5Hash(Item.Url))
                        break;
                }

                //发出请求
                CrawlResponse ItemResponse = null;
                CrawlRequest ItemRequest = Response.Request;
                ItemRequest.Url = Item.Url;
                ItemRequest.TargetUrlPattern = null;
                int retryTime = 0;
                ...
                do
                {
                    ...
                    {
                        // 判断浏览器类型，获取网页Html
                        switch (crawlRecode.BrowserType)
                        {
                            case 1:
                                ItemResponse = GeckoRequestProcessorProxy.GetResponse(ItemRequest, crawlRecode);
                                break;
                            default:
                                ItemResponse = WebRequestProcessor.GetResponse(ItemRequest, crawlRecode);
                                break;
                        }
                    }
                    catch
                    {
                        ItemResponse = new CrawlResponse(
                            ItemRequest, DateTime.Now, Enums.CrawlResult.GeneralConnectError, ItemRequest.Url, null, 0, 1, 1);
                    }
                    retryTime++;
                }
                while ((ItemResponse == null || ItemResponse.Status != Enums.CrawlResult.Succ) && retryTime <= crawlRecode.FailRetry);

                if (ItemResponse != null && ItemResponse.Status == Enums.CrawlResult.Succ && !string.IsNullOrWhiteSpace(ItemResponse.Content))
                {
                    if (ItemResponse.Content.Length < 200)
                    {
                        Logger.Warn(ItemResponse.Content);
                    }

                    //在使用之前识别的xpath获取失败后，考虑直接用基于行文字密度的简单算法提取
                    Article article = Html2Article.GetArticle(ItemResponse.Content);
                    HtmlNode itempagenode = HtmlUtility.getSafeHtmlRootNode(ItemResponse.Content, true, true);

                    //提取文章
                    try
                    {
                        Item.Content = string.Empty;
                        Item.Content = CleanContent(itempagenode.SelectNodes(ListPagePattern.Path.ItemContentXPath), Item.Url);

                    }
                    catch (Exception ex)
                    {
                        Item.Content = article.ContentWithTags;
                    }
                    ...
                    if (string.IsNullOrEmpty(Item.Title))
                        try
                        {
                            Item.Title = string.IsNullOrWhiteSpace(ListPagePattern.Path.ItemTitleXPath) ? article.Title :
                               TextCleaner.FullClean(HTMLCleaner.GetCleanInnerText(itempagenode.SelectSingleNode(ListPagePattern.Path.ItemTitleXPath)));
                        }
                        catch { Item.Title = article.Title; }

                    //确认时间
                    try
                    {
                        DateTime Pubdate = string.IsNullOrWhiteSpace(ListPagePattern.Path.ItemDateXPath) ? article.PublishDate :
                           DateTimeParser.Parser(HTMLCleaner.GetCleanInnerText(itempagenode.SelectSingleNode(ListPagePattern.Path.ItemDateXPath)));
                        if (Item.Pubdate == null && Pubdate.Year > 2000)
                            Item.Pubdate = Pubdate;
                        if (Item.Pubdate.Hour == 0 && Item.Pubdate.Minute == 0 && (Pubdate.Hour != 0 || Pubdate.Minute != 0) && Pubdate.Year > 2000)
                            Item.Pubdate = Pubdate;
                    }
                    catch { Item.Pubdate = article.PublishDate; }
                    //确认媒体
                    if (string.IsNullOrEmpty(Item.MediaName))
                        try
                        {
                            string MediaName = HTMLCleaner.GetCleanInnerText(itempagenode.SelectSingleNode(ListPagePattern.Path.ItemMediaNameXPath));
                            Item.MediaName = CleanMediaName(MediaName);
                        }
                        catch { }
                    //确认作者
                    if (string.IsNullOrEmpty(Item.Author))
                        try
                        {
                            string Author = HTMLCleaner.GetCleanInnerText(itempagenode.SelectSingleNode(ListPagePattern.Path.ItemAuthorXPath));
                            Item.Author = CleanAuthor(Author);
                        }
                        catch { }
                ...
                {
                    Logger.Security("Crawl null content,URL:" + Item.Url);
                }

                ItemResponse IR = new ItemResponse(Response, Enums.CrawlResult.Succ);
                RawItem rawItem = new RawItem(crawlRecode.CrawlID, crawlRecode.CrawlerName, (Enums.ContentDetailLevel)crawlRecode.ContentDetailLevel);

                PageElement.FillIntoRawItem(Item, ref rawItem, crawlRecode);
                IR.DataItem = rawItem;

                if (IR.DataItem != null && !ItemIDs.Contains(IR.DataItem.ItemID))
                {
                    Items.Add(IR);
                    ItemIDs.Add(IR.DataItem.ItemID);
                }
            }
            ...
            List<HTMLLinkItem> Links = null;
            if (!string.IsNullOrEmpty(crawlRecode.PatternFetchUrl) || Items != null && Items.Count > 0)
            {
                if (Items == null)
                    //列表页不获取信息，只拿Url，从Item页抓取,但Crawl.PatternFetchUrl必须不为空
                    Links = !string.IsNullOrEmpty(crawlRecode.PatternFetchUrl) ? null : new List<HTMLLinkItem>(HTMLLinks.LinksFromHTML(new Uri(Response.Url), Response.Content));
                else
                {
                    //列表页获取部分信息，还要抓Item页获取其他信息
                    int UrlErrorCount;
                    Links = HTMLLinks.LinksFromItemResult(new Uri(Response.Url), Items, out UrlErrorCount);
                    //Url错误累计入错误数量
                    ErrorCount += UrlErrorCount;
                }

                //UrlPattern过滤
                if (!string.IsNullOrEmpty(crawlRecode.PatternFetchUrl))
                    Links = HTMLLinks.FilterLinksByPattern(crawlRecode.PatternFetchUrl, Links);

                //清理及去重

                Links = HTMLLinks.CleanUrl(Links);
            }
            ...
            //如果没有内容或全错，则认为List Parse失败
            bool MisMatch = (pageElementList == null || pageElementList.Length == ErrorCount);

            //准备结果
            ListResponse result = new ListResponse(Response, Enums.CrawlResult.Succ, (int)stopWatch.ElapsedMilliseconds
                , pageElementList == null ? 0 : pageElementList.Length, pageElementList == null ? 0 : pageElementList.Length, ErrorCount, Items, Links);

            result.CountExist = (pageElementList == null ? 0 : pageElementList.Length) - (Items == null ? (Links == null ? 0 : Links.Count) : Items.Count);
            result.CountNew = (Items == null ? (Links == null ? 0 : Links.Count) : Items.Count);
        ...
        public static List<ListPagePattern> List_ExtractPattern_n_CrawlItem(string Url, string HTML, Enums.MediaType MediaType, Enums.Language Language, int RandomItemCount)
        {
            ...
            foreach (ListPagePattern XPath in XPaths)
                foreach (PageElement Item in XPath.Content.OrderBy(x => rmd.Next(10000)).Take(RandomItemCount))
                {
                    ...
                    if (article == null || string.IsNullOrWhiteSpace(article.ContentWithTags))
                    {
                        Item.Content = "ItemPage Parse Error";
                        continue;
                    }
                    Item.Content = article.ContentWithTags;
                    if (string.IsNullOrEmpty(Item.Title)) Item.Title = article.Title;
                    if (Item.Pubdate == null && article.PublishDate.Year > 2000) Item.Pubdate = article.PublishDate;
                }
            return XPaths;
        }
        ...
        public static List<ListPagePattern> List_ExtractPattern(string Url, string HTML, Enums.MediaType MediaType, Enums.Language Language, bool NeedExtractContent = true, bool NeedScore = true)
        {
            ...
            //AllFieldsPath = AllFieldsPath.Where(p => p.Score >= AllFieldsPath[0].Score / 3).ToList();

            //if (AllFieldsPath.Count > (int)Strategy.Threshold.ListSave && NeedScore)
            //AllFieldsPath = AllFieldsPath.Take((int)Strategy.Threshold.ListSave).ToList();

            if (NeedExtractContent)
                AllFieldsPath.ForEach(p => p.Content = ExtractContent(Url, rootNode, p.Path, new DateTime(1, 1, 1), false));
        ...
        {
            //调试状态下，判断是否不存在需要提出的a标签
            if (ConfigUtility.IsDebugMode() == true)
            {
                string ii = string.Empty;
                foreach (HtmlNode node in Nodes)
                    if (node.InnerText.Contains("下一页"))
                        ii = node.InnerText;
            }
        ...
        public static ListPagePattern List_getAllFieldsXPath_SVM(string Url, HtmlNode RootNode, HtmlPattern LinkPattern, bool ExtractContent, SoftStrategy Strategy)
        {
            ...
            if (string.IsNullOrWhiteSpace(LinkPattern.RelXPath))
            {
                ...
                if (ExtractContent)
                    lpxp.Content = AIParser.ExtractContent(Url, RootNode, lpxp.Path, DateTime.MinValue);
        ...
        /// <summary>
        /// 这个是照着list页面写出来的东西。承接
        /// </summary>
        /// <param name="ItemPages"></param>
        /// <param name="MediaType"></param>
        /// <param name="Language"></param>
        /// <returns></returns>
        public static ItemPageXPath Item_DownloadnExtractPatterns(List<PageElement> ListContents, SoftStrategy strategy, ref string Statue, bool NeedScore = false)
        {

            //先做一定的检查，如果有问题则返回
            if (ListContents == null || ListContents.Count() < strategy.Threshold.itempageminvalidvalue)
            {
                Statue = "列表页提取文章信息失败，或没有足够多的文章以供识别判断"; return null;
            }

            //先把listxpath中正确的抽取出来;
            List<PageElement> AllPageElements = new List<PageElement>();
            foreach (PageElement pe in ListContents)
                if (pe != null && !string.IsNullOrWhiteSpace(pe.Url) && pe.Url.Contains("http") && !AllPageElements.Exists(p => p.Url == pe.Url))
                    try
                    {
                        pe.HtmlContent = HttpHelper.GetHttpContent(pe.Url);
                        AllPageElements.Add(pe);
                    }
                    catch { }

            if (AllPageElements == null || AllPageElements.Count() < strategy.Threshold.itempageminvalidvalue)
            {
                Statue = "列表页提取文章信息失败，或没有足够多的文章以供识别判断"; return null;
            }

            return Item_ExtractPattern(AllPageElements, strategy, ref Statue, NeedScore);
        }


        /// <summary>
        /// 需要一个写好pageElement中的htmlcontent的list
        /// </summary>
        /// <param name="ListContents"></param>
        /// <param name="strategy"></param>
        /// <param name="Statue"></param>
        /// <param name="NeedScore"></param>
        /// <returns></returns>
        public static ItemPageXPath Item_ExtractPattern(List<PageElement> ListContents, SoftStrategy strategy, ref string Statue, bool NeedScore = false)
        {
            List<ItemPageMessage> ItemPages = new List<ItemPageMessage>();

            #region 合并剪枝

            List<PageElement> PageElements = new List<PageElement>();
            bool success = false;
            Dictionary<string, List<string>> wrongurlpairs = new Dictionary<string, List<string>>();
            string wrongitemurl1 = string.Empty;
            string wrongitemurl2 = string.Empty;
            int timesfoundwrongpairs = 0;

            while (!success && timesfoundwrongpairs < 20)//尝试20次组合，如果失败则返回
                try
                {
                    int trycount = 0;

                    //直接减少数量
                    List<PageElement> ResultPageElements = new List<PageElement>();
                    while (trycount < 100 && ResultPageElements.Count() < strategy.Threshold.itempagemaxvalue)
                    {
                        //重新洗牌，尝试按当前的“不可能配对”信息进行抽取
                        ListContents.OrderBy(x => rmd.Next(10000)); ResultPageElements = new List<PageElement>();
                        foreach (PageElement PE in ListContents)
                        {
                            bool isurlgood = true;
                            foreach (PageElement pe in ResultPageElements)
                                if (wrongurlpairs.ContainsKey(pe.Url))
                                    if (wrongurlpairs[pe.Url].Contains(PE.Url))
                                        isurlgood = false;
                            if (isurlgood) ResultPageElements.Add(PE);
                            if (ResultPageElements.Count() == strategy.Threshold.itempagemaxvalue) break;
                        }
                        trycount++;
                    }

                    PageElements = ResultPageElements;//如果尝试50次后归结出来不到三个当然也是没法做的了，直接返回失败吧
                    if (PageElements == null || PageElements.Count() < strategy.Threshold.itempageminvalidvalue)
                    {
                        Statue = "列表页提取文章信息失败，或没有足够多的文章以供识别判断"; return null;
                    }

                    //规范化，写成itempagemessage的内容
                    foreach (PageElement ContentElement in PageElements)
                        ItemPages.Add(new ItemPageMessage(ContentElement.Url, ContentElement.HtmlContent));



                    //cut之前保存数据，与Cut后的数据对比，检查是否成功简化
                    if (ItemPages.Count > 2)
                    {
                        string Title = string.Empty;
                        int i = 1;

                        foreach (ItemPageMessage itp in ItemPages)
                            foreach (ItemPageMessage itp2 in ItemPages)
                                if (itp.Url != itp2.Url)
                                {
                                    wrongitemurl1 = itp.Url;
                                    wrongitemurl2 = itp2.Url;
                                    itp.RootNode = HTMLCleaner.DOMCompareCleaner(HtmlUtility.getSafeHtmlRootNode(itp.Html), HtmlUtility.getSafeHtmlRootNode(itp2.Html), ref i);
                                }
                    }
                    success = true;
                }
                ...
                {
                    if (!string.IsNullOrWhiteSpace(wrongitemurl1) && !string.IsNullOrWhiteSpace(wrongitemurl2))
                    {
                        timesfoundwrongpairs++;
                        //好像用不到这个计数。如果前面错误太多，到这里添加字典的时候就会报错了。
                        if (!wrongurlpairs.ContainsKey(wrongitemurl1))
                            wrongurlpairs.Add(wrongitemurl1, new List<string>());
                        wrongurlpairs[wrongitemurl1].Add(wrongitemurl2);
                        if (!wrongurlpairs.ContainsKey(wrongitemurl2))
                            wrongurlpairs.Add(wrongitemurl2, new List<string>());
                        wrongurlpairs[wrongitemurl2].Add(wrongitemurl1);
                    }
                    ItemPages = null;
                }
            ...
            if (!success)
            {
                Statue = "文章页简并失败"; return null;
            }

            foreach (ItemPageMessage itpm in ItemPages)
                itpm.NodeCount = Item_CountChildNodes(itpm.RootNode);

            #endregion 合并剪枝


            //生成用于比较正文内容的几个页面根节点
            Dictionary<string, HtmlNode> ComPareRootNodes = new Dictionary<string, HtmlNode>();
            foreach (PageElement pe in ListContents.OrderBy(x => rmd.Next(10000)).Take(3))
            {
                HtmlNode rootnode = HtmlUtility.getSafeHtmlRootNode(pe.HtmlContent, true, true);
                if (rootnode != null)
                    ComPareRootNodes.Add(pe.Url, rootnode);
            }


            //进行提取和判断
            ItemPageXPath ItemXPaths = new ItemPageXPath();
            if (strategy.MediaType == Enums.MediaType.Forum)
                ItemXPaths = Item_LinkPattern_getCandidate_Forum(ItemPages, PageElements, ComPareRootNodes, strategy, NeedScore);
            else
                ItemXPaths = Item_LinkPattern_getCandidate_WebNews(ItemPages, PageElements, ComPareRootNodes, strategy, NeedScore);


            if (ItemXPaths == null) return null;
            else return ItemXPaths;
            //ItemPagesforSaving.AddRange(ItemPages);
            //List<ItemPageXPath> ItemPatterns = new List<ItemPageXPath>();
            //return ItemPatterns;
        ...
        /// <param name="Language"></param>
        /// <returns></returns>
        public static ItemPageXPath Item_ExtractPatternWithPattern(ListPagePattern ListXPath, SoftStrategy strategy, ref string Statue, bool NeedScore = false)
        {

            //先做一定的检查，如果有问题则返回
            if (ListXPath.Content == null || ListXPath.Content.Count() < strategy.Threshold.itempageminvalidvalue)
            {
                Statue = "列表页提取文章信息失败，或没有足够多的文章以供识别判断"; return null;
            }
            ...
            //先把listxpath中正确的抽取出来;
            List<PageElement> AllPageElements = new List<PageElement>();
            foreach (PageElement pe in ListXPath.Content)
                if (pe != null && !string.IsNullOrWhiteSpace(pe.Url) && pe.Url.Contains("http") && !AllPageElements.Exists(p => p.Url == pe.Url))
                    AllPageElements.Add(pe);
            if (AllPageElements == null || AllPageElements.Count() < strategy.Threshold.itempageminvalidvalue)
            {
            ...
            while (!success && timesfoundwrongpairs < 10)//尝试10次组合，如果失败则返回
                try
                {
                    int trycount = 0;
                    ...
                    {
                        //重新洗牌，尝试按当前的“不可能配对”信息进行抽取
                        AllPageElements.OrderBy(x => rmd.Next(10000)); ResultPageElements = new List<PageElement>();
                        foreach (PageElement PE in AllPageElements)
                        {
                            bool isurlgood = true;
                            foreach (PageElement pe in ResultPageElements)
                                if (wrongurlpairs.ContainsKey(pe.Url))
                    ...
                    //规范化，写成itempagemessage的内容
                    foreach (PageElement ContentElement in PageElements)
                        ItemPages.Add(new ItemPageMessage(ContentElement.Url, HttpHelper.GetHttpContent(ContentElement.Url)));
            ...
            //生成用于比较正文内容的几个页面根节点
            Dictionary<string, HtmlNode> ComPareRootNodes = new Dictionary<string, HtmlNode>();
            foreach (PageElement pe in ListXPath.Content.Take(3))
            {
                HtmlNode rootnode = HtmlUtility.getSafeHtmlRootNode(HttpHelper.GetHttpContent(pe.Url), true, true);
                if (rootnode != null)
                    ComPareRootNodes.Add(pe.Url, rootnode);
            }
        ...
        public static ItemPageXPath Item_ExtractPattern(List<PageElement> ListContents, SoftStrategy strategy, bool NeedScore = false)
        {
            string Statue = "";
            return Item_ExtractPattern(ListContents, strategy, ref Statue);
        }

        public static ItemPageXPath Item_ExtractPatternWithPattern(ListPagePattern ListXPath, SoftStrategy strategy, bool NeedScore = false)
        {
            string Statue = "";
            return Item_ExtractPatternWithPattern(ListXPath, strategy, ref Statue);
        }
        ...
        /// <param name="Language"></param>
        /// <returns></returns>
        public static ItemPageXPath Item_LinkPattern_getCandidate_WebNews(IEnumerable<ItemPageMessage> ItemPages, List<PageElement> PageElements, Dictionary<string, HtmlNode> ComPareNodes, SoftStrategy strategy, bool NeedScore = false)
        {
            ...
            ItemPageXPath Result = new ItemPageXPath();
            ...
            ItemPageXPath itmpx = new ItemPageXPath();
            itmpx = Result;
            ...
            {
                PageElement content = PageElements.FirstOrDefault(c => c.Url == url);
                ...
                string reply = content.Reply > 0 ? string.Empty : content.Reply.ToString();
                ReplyXPath = Item_GetStringXPaths(BasicPatterns[url], reply, RootNodes[url], Result.ReplyXPath, PatternType.Reply);

                string view = content.View > 0 ? string.Empty : content.View.ToString();
                ...
                //Dictionary<PatternType, List<ItemPatternAnaly>> RelPatternsinSomeUrl = Item_CheckNodesForRelXPath(BasicPatterns[url], url, RelString);
                //foreach (PatternType pt in RelPatternsinSomeUrl.Keys)
                //{
                //    if (!ShortRelXPaths.Keys.Contains(pt))
                //        ShortRelXPaths.Add(pt, new Dictionary<string, List<string>>());
                //    if (!ShortRelXPaths[pt].Keys.Contains(url))
                //        ShortRelXPaths[pt].Add(url, new List<string>());

                //    if (!ItmpaForRelXPaths.Keys.Contains(pt))
                //        ItmpaForRelXPaths.Add(pt, new Dictionary<string, List<ItemPatternAnaly>>());
                //    if (!ItmpaForRelXPaths[pt].Keys.Contains(url))
                //        ItmpaForRelXPaths[pt].Add(url, new List<ItemPatternAnaly>());

                //    ItmpaForRelXPaths[pt][url].AddRange(RelPatternsinSomeUrl[pt]);


                //    foreach (ItemPatternAnaly itmpa in RelPatternsinSomeUrl[pt])
                //    {
                //        string xpath = itmpa.XPath;
                //        string substring = string.Empty;
                //        if (xpath.Contains("#"))
                //        {
                //            substring = xpath.Substring(xpath.IndexOf("/#"));
                //            xpath = xpath.Remove(xpath.IndexOf("/#"));
                //        };
                //        HtmlNode node = RootNodes[url].SelectNodes(xpath).ToList().First(r => true);
                //        if (node == null) itmpa.XPath = null;
                //        xpath = XPathUtility.GetrawXPath(node, RootNodes[url], 0, url, true);
                //        if (!string.IsNullOrEmpty(substring)) xpath = xpath + substring;


                //        ShortRelXPaths[pt][url].Add(xpath);
                //    }
                //}
                #endregion useless
            }
        ...
        public static ItemPageXPath Item_LinkPattern_getCandidate_Forum(IEnumerable<ItemPageMessage> ItemPages, List<PageElement> PageElements, Dictionary<string, HtmlNode> ComPareNodes, SoftStrategy strategy, bool NeedScore = false)
        {
            return new ItemPageXPath();
        }
