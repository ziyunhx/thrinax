...
namespace Thrinax
{
    ...
    public class SoftStrategy
    {
        ...
        public SoftStrategy(Enums.MediaType MediaType, Enums.Language Language)
        {
            ...
            //todo:把这里改掉，不能在未来每个都用新闻的模型 -- Done By Fisher 2016-08-15,但是模型的得分怎么确定呢？
            //this.MediaType = Enums.MediaType.WebNews;
            //this.FieldScore = new FieldScoreStrategy(MediaType, Language);

            this.model_title = Model.Read("model/model_title_WebNews");// + MediaType.ToString());
            this.model_rel = Model.Read("model/model_rel_WebNews");// + MediaType.ToString());
        ...
        /// <returns></returns>
        public bool ValidateListXPath(string Url, string HTML, ListPagePattern XPath)
        {
            ...
            HtmlNodeCollection rootNodes = rootNode.SelectNodes(XPath.Path.ItemRootXPath);

            if (rootNodes == null)
                return false;

            var TitleNode = rootNodes.Select(f => f.SelectSingleNode(XPath.Path.TitleXPath)).Where(f => f != null);
        ...
        /// <param name="XPath"></param>
        /// <returns></returns>
        public bool ValidateListXPath(string Url, HtmlNode RootNode, ListPagePattern XPath)
        {
            if (string.IsNullOrEmpty(Url) || RootNode == null || XPath == null) return false;

            PageElement[] Content = ExtractContent(Url, RootNode, XPath.Path, DateTime.MinValue);
            ...
            foreach (PageElement ele in Content)
            {
                if (!string.IsNullOrEmpty(ele.Title) && !string.IsNullOrEmpty(ele.Url)) TitleCount++;
                if (!string.IsNullOrEmpty(XPath.Path.DateXPath) && ele.Pubdate != null) DateCount++;
                if (!string.IsNullOrEmpty(XPath.Path.ViewXPath) && ele.View >= 0) ViewCount++;
                if (!string.IsNullOrEmpty(XPath.Path.ReplyXPath) && ele.Reply >= 0) ReplyCount++;
                if (!string.IsNullOrEmpty(XPath.Path.MediaNameXPath) && !string.IsNullOrEmpty(ele.MediaName)) MediaCount++;
                if (!string.IsNullOrEmpty(XPath.Path.AuthorXPath) && !string.IsNullOrEmpty(ele.Author)) AuthorCount++;
            }

            if (TitleCount < Content.Length * 0.9) return false;
            if (!string.IsNullOrEmpty(XPath.Path.DateXPath) && DateCount < Content.Length * 0.9) return false;
            if (!string.IsNullOrEmpty(XPath.Path.ViewXPath) && ViewCount < Content.Length * 0.9) return false;
            if (!string.IsNullOrEmpty(XPath.Path.ReplyXPath) && ReplyCount < Content.Length * 0.9) return false;
            if (!string.IsNullOrEmpty(XPath.Path.MediaNameXPath) && MediaCount < Content.Length * 0.9) return false;
            if (!string.IsNullOrEmpty(XPath.Path.AuthorXPath) && AuthorCount < Content.Length * 0.9) return false;
